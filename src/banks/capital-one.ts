...
const kafka = new Kafka({
  brokers,
  clientId: 'capital-one-quotes',
  logLevel: logLevel.NOTHING,
});

const producer = kafka.producer();
const consumer = kafka.consumer({ groupId: 'capital-one-quotes-consumers' });

const APPROVAL_RATE = 0.75;
const BANK_NAME = 'Capital One';
...
const run = async (): Promise<void> => {
  ...
  logger.info('started capital-one');
